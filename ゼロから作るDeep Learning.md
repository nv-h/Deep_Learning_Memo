...
ニューラルネットワークの活性化関数にはここで述べる「シグモイド関数」や「ステップ関数」など非線形なものを用いる。
非線形なものを重ねることで複雑な表現をすることができるようになる。逆に線形関数の場合、いくら重ねても複雑な出力を得ることはできない(同じ出力をする隠れ層のないネットワークが存在する)。
...
ステップ関数(階段関数)は、ある閾値を境にして出力が切り替わる関数のこと。パーセプトロンでは活性化関数にステップ関数を採用している。
詳細は前述のとおりなので省略。

#### 3.2.7 ReLU関数

ReLU (Rectified Linear Unit) は以下の数式で表される。0より小さい部分は0になるだけの活性化関数。シグモイド関数に変わって最近良く使用されている。
...
手書き文字認識であるMNISTの実装について述べていたのでここではかなりの部分を省略。
...
プログラムでの微分は基本的には解析的に計算するのではなく、数値微分で近似的に求める。
ある重み入力時の損失関数の微分値を求めることで傾きを求め、入力変数である重みをどのように変化させれば損失関数の出力が減るかを知ることができる。
また、複数の変数がある場合は偏微分を求めることで一つの変数に着目した損失関数の傾きを知ることができる。

### 4.4. 勾配

複数の重みで損失関数が定義される場合、前述のとおり各変数で偏微分することで各変数ごとの傾きを知ることができる。
このときの傾きを合成することで、二次元的、三次元的に傾きを表現することができる。
このような多次元の傾き情報を勾配と呼び、1変数の時と同じように損失関数の出力が小さくなる方向を知ることができる。
...
### 5.6. Affine / Softmax レイヤ

Affineレイヤの順方向は、単に行列の積(重みの乗算)とバイアスの加算で構成される。(逆方向については省略)
...
Softmaxレイヤは、入力された値を正規化して出力する層である。
例えばネットワークの出力であるスコアが`5.3, 0.3, 10.1, ... 0.01`だっとき、これをソフトマックス層に入力すると`0.08, 0.00005, 0.991, ... 0.00004`という正規化した結果(確率として解釈できる)を出力する。
ソフトマックス層は出力の見やすさにしか関わらないので最大値だけ知りたい場合は不要だが、ネットワークの学習時には必須になる。

損失関数として交差エントロピー誤差を入れたネットワークは以下の様になり(mermaidで上手く描けなかった...)、逆伝播が$y_1 - t_1$という「きれいな」結果になる。
これは、そのように交差エントロピー誤差を作ったからであり、こうすることでネットワークの学習を簡単にすることができる。
...
graph LR
    ...
    subgraph Softmax-with-Loss
        a1 --> Softmax --"y1"--> 交差エントロピー誤差
        a2 --> Softmax --"y2"--> 交差エントロピー誤差
        a3 --> Softmax --"y3"--> 交差エントロピー誤差

        交差エントロピー誤差 == "- t1/y1" ==> Softmax == "y1 - t1" ==> a1
        交差エントロピー誤差 == "- t2/y2" ==> Softmax == "y2 - t2" ==> a2
        交差エントロピー誤差 == "- t3/y3" ==> Softmax == "y3 - t3" ==> a3
    end
